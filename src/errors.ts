...
export type AppErrorCode = 'invalid_request' | 'request_rejected' | 'key_unavailable' | 'github_api_error';
...
const HTTP_STATUS: Readonly<Record<AppErrorCode, HttpErrorStatus>> = {
    ...
    // repos outside the installation and for over-broad permissions
    // (confirmed against the live API for both cases). request_rejected
    // passes GitHub's own message straight through rather than pre-checking
    // either condition with extra API calls beforehand.
